...
import { EVENT_ID } from '../constants/eventConstants'
...
export default class extends Component {
  static async getInitialProps() {
    ...
    const getEvent = await fetch(`${picaticHost}/event/${EVENT_ID}`)
    const eventJSON = await getEvent.json()
    const event = await eventJSON.data

    const getSponsors = await fetch(
      `${picaticHost}/sponsor?filter[event_id]=${EVENT_ID}${picaticLimit}`,
    )
